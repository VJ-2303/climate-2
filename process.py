#!/usr/bin/env python3
"""
HeatViz / ThermalGuard Data Preprocessing Pipeline (Script 00)
Preprocesses raw Sentinel-2, Landsat 8/9, OpenStreetMap, WorldPop, and Boundary data
into standardized 20m analysis-ready rasters aligned to target UTM CRS.
"""
...
# Geospatial parameters (Madurai: UTM Zone 43N)
TARGET_CRS = "EPSG:32643"  # UTM Zone 43N (Metric, Madurai)
...
def extract_sentinel_safe():
    ...
    safe_zips = glob.glob(os.path.join(RAW_DIR, "S2*.SAFE.zip")) or glob.glob(os.path.join(RAW_DIR, "S2*.zip"))
    if not safe_zips:
    ...
    with zipfile.ZipFile(safe_zip, 'r') as zip_ref:
        for member in zip_ref.namelist():
            for pattern in band_mapping.keys():
                if pattern in member and 'IMG_DATA' in member:
...
def standardize_landsat_and_worldpop():
    ...
    # Check for population zip or tif
    pop_target = os.path.join(RAW_DIR, "worldpop_population.tif")
    if not os.path.exists(pop_target):
        pop_tifs = glob.glob(os.path.join(RAW_DIR, "ind_pop*.tif")) or glob.glob(os.path.join(RAW_DIR, "ken_ppp_2020*.tif"))
        if not pop_tifs:
            pop_zips = glob.glob(os.path.join(RAW_DIR, "ind_pop*.zip"))
            if pop_zips:
                print(f"Extracting population zip: {os.path.basename(pop_zips[0])}...")
                with zipfile.ZipFile(pop_zips[0], 'r') as z:
                    for name in z.namelist():
                        if name.endswith('.tif'):
                            z.extract(name, RAW_DIR)
                pop_tifs = glob.glob(os.path.join(RAW_DIR, "ind_pop*.tif"))

        if pop_tifs:
            src_pop = [f for f in pop_tifs if os.path.basename(f) != "worldpop_population.tif"][0]
            shutil.copyfile(src_pop, pop_target)
            print(f"✓ Copied {os.path.basename(src_pop)} -> worldpop_population.tif")


def split_osm_data():
    """Split OSM complete GeoJSON into 4 categorized GeoJSON files."""
    target_files = [
    ...
    osm_candidates = [
        os.path.join(RAW_DIR, "madurai_complete.geojson"),
        os.path.join(RAW_DIR, "export.geojson")
    ]
    osm_export = next((c for c in osm_candidates if os.path.exists(c)), None)

    if not osm_export:
        print("! Warning: No OSM export GeoJSON found in data/raw.")
        return

    print(f"Splitting OSM {os.path.basename(osm_export)} into feature layers...")
...
def build_master_grid_and_process():
    """
    Construct the unified 20m target CRS master raster grid covering 100% of the
    boundary plus buffer, and reproject/resample all layers onto this grid.
    """
    print("\n" + "=" * 60)
    print(f"Executing Master Processing Pipeline ({TARGET_CRS} @ {TARGET_RESOLUTION}m)")
    print("=" * 60)

    # 1. Process Boundary
    print("\n[1/7] Processing Study Area Boundary...")
    boundary_candidates = [
        os.path.join(RAW_DIR, "madurai_boundary.json"),
        os.path.join(RAW_DIR, "madurai_boundary.geojson"),
    ]
    boundary_raw_path = next((b for b in boundary_candidates if os.path.exists(b)), None)
    if not boundary_raw_path:
        raise FileNotFoundError("No boundary file found in data/raw.")

    boundary = gpd.read_file(boundary_raw_path)
    boundary_utm = boundary.to_crs(TARGET_CRS)
    boundary_utm.to_file(os.path.join(PROCESSED_DIR, "madurai_boundary.geojson"), driver="GeoJSON")
    print(f"✓ Reprojected boundary ({os.path.basename(boundary_raw_path)}) to {TARGET_CRS}")
    ...
    def save_master_raster(arr, out_name, nodata=NODATA_VAL, dtype='float32'):
        ...
        min_v = float(np.nanmin(valid_vals)) if len(valid_vals) > 0 else 0.0
        max_v = float(np.nanmax(valid_vals)) if len(valid_vals) > 0 else 0.0
        mean_v = float(np.nanmean(valid_vals)) if len(valid_vals) > 0 else 0.0
    ...
    buildings_path = os.path.join(RAW_DIR, "osm_buildings.geojson")
    if os.path.exists(buildings_path):
        buildings_gdf = gpd.read_file(buildings_path).to_crs(TARGET_CRS)
    else:
    ...
    roads_path = os.path.join(RAW_DIR, "osm_roads.geojson")
    if os.path.exists(roads_path):
        roads_gdf = gpd.read_file(roads_path).to_crs(TARGET_CRS)
    else:
    ...
    green_path = os.path.join(RAW_DIR, "osm_green.geojson")
    green_gdf = gpd.read_file(green_path).to_crs(TARGET_CRS) if os.path.exists(green_path) else gpd.GeoDataFrame()
    if len(green_gdf) > 0:
    ...
    water_path = os.path.join(RAW_DIR, "osm_water.geojson")
    water_gdf = gpd.read_file(water_path).to_crs(TARGET_CRS) if os.path.exists(water_path) else gpd.GeoDataFrame()
    if len(water_gdf) > 0:
    ...
    pop_raw_path = os.path.join(RAW_DIR, "worldpop_population.tif")
    if os.path.exists(pop_raw_path):
        pop_raw = reproject_to_master(pop_raw_path, resampling_method=Resampling.bilinear)
    ...
    for opt_name, filename in [("ndvi", "ndvi.tif"), ("ndwi", "ndwi.tif"), ("ndbi", "ndbi.tif")]:
        ...
        if np.any(opt_missing) and np.any(valid_mask):
    ...
    if np.any(st_missing) and np.any(valid_st):
        print(f"  Training XGBoost thermal inpainting on {np.sum(valid_st):,} clear pixels to reconstruct {np.sum(st_missing):,} cloud-masked pixels...")
        full_features = [
            loaded_rasters["ndvi"],
            loaded_rasters["ndwi"],
            loaded_rasters["ndbi"],
